            ...
            <!-- <div class="form-group">
              <div class="mb-3">
                ...
                <div class="input-group mb-3">
                    <input class="form-control" name="visions[]" type="text" 
                      value="<?php echo (is_array($value->visions)) ? $value->visions[0] : null ?>">
                    
                    <div class="input-group-append">
                        <button type="button" class="addVision btn btn-primary"><i class="fa fa-plus"></i></button>
                    </div>
                </div>

                <?php if(is_array($value->visions)) { ?>
	  							<?php for($x = 1; $x<count($value->visions); $x++) { ?>
                    <div class="input-group mb-3">
                        <input class="form-control" name="visions[]" type="text" value="<?php echo $value->visions[$x] ?>">
                        <div class="input-group-append">
                          <button type="button" class="removeVision btn btn-danger"><i class="fa fa-times"></i></button>
                        </div>
                    </div>
                  <?php } ?>
                <?php } ?>
...
<script>
  $(document).ready(function() {
    var x = 1;
    $(document).on('click', '.addVision', function() {
        x++;
        $(".inputMission").append(`
            <div class="input-group mb-3">
                <input class="form-control" name="visions[]" type="text" value="">
                <div class="input-group-append">
                  <button type="button" class="removeVision btn btn-danger"><i class="fa fa-times"></i></button>
                </div>
            </div>`
